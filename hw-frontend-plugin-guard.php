...
function hw_block_biteship_now(): bool {
    static $block = null;
    if ($block === null) {
        $block = !hw_allow_biteship_now();
    }
    return $block;
}
function hw_block_yith_now(): bool {
    static $block = null;
    if ($block === null) {
        $block = !hw_allow_yith_now();
    }
    return $block;
}
...
function hw_unhook_all_from_dir(string $dir_fragment, array $hooks, array $name_hints = []){
    global $wp_filter;
    static $file_cache = [];
    $dir_fragment = strtolower($dir_fragment);
    $normalized_hints = [];
    ...
    foreach ($hooks as $hook) {
        ...
        foreach ($wp_filter[$hook]->callbacks ?? [] as $prio => $callbacks) {
            foreach ($callbacks as $cb_id => $cb) {
                ...
                if ($file) {
                    $file_l = strtolower((string) $file);
                    if (strpos($file_l, $dir_fragment) !== false) {
                        remove_filter($hook, $callable, $prio);
                    }
                }
            }
...
add_action('init', function(){
    if (!hw_block_biteship_now()) return; // Biarkan di checkout/admin
...
add_filter('script_loader_src', function($src){
    ...
    if ($block === null) {
        $block = [
            'biteship' => hw_block_biteship_now(),
            'yith'     => hw_block_yith_now(),
        ];
        if (!$block['biteship'] && !$block['yith']) {
            remove_filter('script_loader_src', __FUNCTION__, 9999);
            return $src;
        }
    }
    if ($block['biteship'] && stripos($src, '/wp-content/plugins/biteship/') !== false) return false;
    if ($block['yith']     && stripos($src, '/wp-content/plugins/yith-point-of-sale-for-woocommerce-premium/') !== false) return false;
    return $src;
}, 9999);
add_filter('style_loader_src', function($src){
    ...
    if ($block === null) {
        ...
        if (!$block['biteship'] && !$block['yith']) {
            remove_filter('style_loader_src', __FUNCTION__, 9999);
            return $src;
        }
    }
    if ($block['biteship'] && stripos($src, '/wp-content/plugins/biteship/') !== false) return false;
...
add_action('wp_enqueue_scripts', function(){
    if (hw_block_biteship_now()) hw_kill_handles_like('biteship');
    if (hw_block_yith_now())     hw_kill_handles_like('yith-pos');
}, 9999);
...
function hw_buffer_strip_biteship($html){
    if (!hw_block_biteship_now()) return $html;
...
add_action('template_redirect', function(){
    if (!hw_block_biteship_now() || is_admin()) return;
